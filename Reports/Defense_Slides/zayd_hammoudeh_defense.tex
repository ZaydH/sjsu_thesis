...
\begin{frame}{Previous Work}{Paikin \& Tal}
  \onslide<1->{
    ...
    \begin{itemize}
      ...
      \item Supports Type~1 and Type~2 puzzles
...
\begin{frame}{Introduction}{Thesis Overview}
...
\begin{frame}{Mixed-Bag Solver}{Overview}
  ...
  \end{itemize}
  \begin{center}
    \fbox{\includegraphics[width=0.9\textwidth]{./images/cropped_algorithm_structure_overview.pdf}}
    \vfill    
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%
...
\begin{frame}{Mixed-Bag Solver}{Final Assembly}
  \begin{itemize}
    \item \textbf{Input}: Seed pieces
	\vspace{1em}
    \item \textbf{Output}: Final solved puzzles
    \vfill
    \item \textbf{Summary}: In this stage, the assembler places all pieces around the previously selected seed pieces.
    \vfill
    \item Assembly proceeds normally in this stage with no custom modifications.
  \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%
...
\begin{frame}{Quantifying Solver Quality}{Standard and Enhanced Direct Accuracy}
  \onslide<1->{
    \begin{itemize}
      \item \textbf{Direct Accuracy}: Fraction of pieces ($c$) placed in the same location in both the ground-truth and solved image with respect to the total number of pieces ($n$)
    \end{itemize}
    \vfill
...
\begin{frame}{Direct Accuracy}{Effect of Shifts}
  ...
  \begin{figure}
    \centering
    \begin{tabular}{ 
      >{\centering\arraybackslash}m{0.45\textwidth} >{\centering\arraybackslash}m{0.47\textwidth} }
	    \onslide<2->{
	      \fbox{\includegraphics[width=0.42\textwidth]{./images/muffins_300x200.jpg}}
	    } &
	    \onslide<3->{
	      \fbox{\includegraphics[width=0.45\textwidth]{./images/muffins_300x200_type1}}
	    }
	    \\ ~\\
	    \onslide<2->{
	      Ground-Truth Image
...
\begin{frame}{Performance Comparison on Multiple Input Puzzles}{Results Summary}
  \begin{itemize}
    \onslide<1->{
