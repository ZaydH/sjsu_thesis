...
\begin{frame}{Introduction}{Thesis Goals}
  ...
  \onslide<2->{
    ...
    \begin{itemize}
      ...
      \item Define the first metrics that quantify the quality of outputs from a multiple puzzle solver
...
\onslide<1->{There are three primary jig swap puzzle types as formalized by~\cite{gallagher2012}. In all cases, the ``ground-truth'' input is unknown.} 
    \vfill
    \begin{itemize}
        \item<2-> \textbf{Type~1}: Puzzle dimension and piece rotation are known.  May have ``anchor'' piece(s) fixed in the correct location(s).
        \vfill
...
\begin{frame}{Mixed-Bag Solver}{Foundations}
	...
	\end{block}}
	\vfill
  \onslide<4->{The latter strategy is the basis of the {\color{spartanBlue}\textbf{Mixed-Bag Solver}}}
\end{frame}
  ...
  \begin{center}
	  \begin{tabular}{ 
	    >{\centering\arraybackslash}m{0.8\textwidth}}
		    ...
		    \fbox{\includegraphics[scale=0.13]{./images/segmentation/pomeranz_805_8.jpg}}
		    \\[-0.2em]~\\[-0.2em]
		    Image~(a) \textendash { }805~Pieces~\cite{pomeranzBenchmarkImages}
	      \\~\\[-0.2em]
		    \fbox{\includegraphics[scale=0.13]{./images/segmentation/mcgill_540_20.jpg}}
	      \\[-0.2em]~\\[-0.2em]
...
\begin{frame}{Segmentation}{Example -- First Segmentation Round Output Image}
  \begin{center}
	  \begin{tabular}{ 
	    ...
	    \fbox{\includegraphics[scale=0.13]{./images/segmentation/assembler_single_puzzle_output.jpg}} 
	  \end{tabular}
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%
...
\begin{frame}{Segmentation}{Example -- Segmented Output Image}
  \begin{center}
	  \begin{tabular}{ 
	    >{ \centering\arraybackslash}m{0.9\textwidth} }
	    \fbox{\includegraphics[scale=0.13]{./images/segmentation/segmented_single_puzzle_output.jpg}} 
	  \end{tabular}
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%
...
\begin{frame}{Stitching}{Mixed-Bag Solver Stage \#2}
  \begin{itemize}
    \onslide<1->{
	  ...
	  \vfill
	  \onslide<2->{
	    \item \textbf{Theoretical Foundation}: If two segments from the same ground-truth image, they would eventually \textbf{overlap} if one segment were to expand.
	    \vspace{0.4em}
	    \begin{itemize}
	      \setlength\itemsep{0.6em}
...
\begin{frame}{Stitching}{Example -- Two Segment Images}
  ...
  {\small 
  \begin{center}
	  \begin{tabular}{>{\centering\arraybackslash}b{0.2\textwidth} >{\centering\arraybackslash}b{0.78\textwidth} }
...
\begin{frame}{Hierarchical Segment Clustering}{Example -- Single Linking}
  ...
  \setlength{\tabcolsep}{0pt}
	  \begin{tabular}{>{\centering\arraybackslash}b{0.17\textwidth} >{\centering\arraybackslash}b{0.24\textwidth}>{\centering\arraybackslash}b{0.20\textwidth}>{\centering\arraybackslash}b{0.185\textwidth}}
...
\begin{frame}{Hierarchical Segment Clustering}{Example -- Single Linking}
  ...
  \setlength{\tabcolsep}{0pt}
	  \begin{tabular}{>{\centering\arraybackslash}b{0.45\textwidth} >{\centering\arraybackslash}b{0.45\textwidth}}
		   \fbox{\includegraphics[scale=0.25]{./images/clustering/mcgill_20_segment_1.jpg}
		    \hspace{0.1in} \includegraphics[scale=0.25]{./images/clustering/mcgill_20_segment_2.jpg}}
		    & \fbox{\includegraphics[scale=0.25]{./images/clustering/mcgill_20_segment_3.jpg}
		    \hspace{0.1in} \includegraphics[scale=0.25]{./images/clustering/mcgill_20_segment_4.jpg}}
		    \\~\\
...
\begin{frame}{Hierarchical Segment Clustering}{Terminating Clustering}
  \begin{itemize}
  	\item The solver continues merging segment clusters until one of two criteria are satisfied:
	  \vspace{0.4em}
	  \begin{itemize}
	    \setlength\itemsep{0.8em}
	    \item Only a single segment cluster remains
	    \item Maximum similarity between any segment clusters is below a predefined threshold~(e.g.,~0.1)
	  \end{itemize} 
	  \vfill
  ...
  \begin{figure}
    ...
    \begin{tabular}{ 
      >{\centering\arraybackslash}m{0.44\textwidth} >{\centering\arraybackslash}m{0.47\textwidth} }
	    ...
	    \onslide<3->{
	      \fbox{\includegraphics[width=0.44\textwidth]{./images/muffins_300x200_type1.jpg}}
	    }
	    \\ ~\\
	    \onslide<2->{
	      Ground-Truth Image
...
\begin{frame}{Direct Accuracy}{Example -- Shiftable Reference Point}
  \begin{figure}
    ...
    \begin{tabular}{ 
      >{\centering\arraybackslash}m{0.44\textwidth} >{\centering\arraybackslash}m{0.47\textwidth} }
	    \onslide<1->{
	      \fbox{\includegraphics[width=0.42\textwidth]{./images/muffins_300x200_type1.jpg}}
	    } &
	    \only<2>{\fbox{\includegraphics[width=0.42\textwidth]{./images/muffins_300x200_type1_EDAS.jpg}}}\only<3>{\fbox{\includegraphics[width=0.42\textwidth]{./images/muffins_300x200_type1_SEDAS.jpg}}}
	    \\ ~\\
	    \onslide<1->{
	      {\small Solver Output}
	    } & 
	    \only<2>{\small Direct Accuracy Reference Point}\only<3>{\small SEDAS Reference Points} 
	    \\ ~\\
  \end{tabular}
  \end{figure}
\end{frame}
...
\begin{frame}{Determining Input Puzzle Count}{Visual Comparison of a Misclassified Image}
  \begin{tabular}{ >{\centering\arraybackslash}m{0.465\textwidth} >{\centering\arraybackslash}m{0.465\textwidth} }
	  \fbox{\includegraphics[width=0.43\textwidth]{./images/single_puzzle/pomeranz_805_14.jpg}}
	  &
	  \fbox{\includegraphics[width=0.43\textwidth]{./images/single_puzzle/pomeranz_805_12.jpg}}  
	  \\~\\	
...
\onslide<1->{There are four primary jig swap puzzle types as formalized by~\cite{gallagher2012}. In all cases, the ``ground-truth'' input is unknown.} 
    ...
    \begin{itemize}
        ...
        \vfill
        \item<5-> \textbf{Mixed-Bag}: The set of puzzle pieces come from multiple puzzles.
    \end{itemize}
    \vfill
...
\begin{figure}[t]
  ...
  \begin{tabular}{ >{\centering\arraybackslash}m{0.40\textwidth} >{\centering\arraybackslash}m{0.40\textwidth} }
     ...
     \\~\\
     \onslide<1->{Original Image~\cite{pixabay}} & \onslide<2->{Best Buddy Visualization}
  \end{tabular}
