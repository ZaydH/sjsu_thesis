...
\begin{frame}{Segmentation}{Mixed-Bag Solver Stage \#1}
  \begin{itemize}
      ...
      \vspace{0.4em}
	    \item \textbf{Role of Segmentation}: Provide basic structure to the set of puzzle pieces by partitioning them into disjoint segments
	    \begin{itemize}
	      \vspace{0.4em}
      	\item \textbf{Input}: Set of puzzle pieces
  		  \vspace{0.6em}
...
\begin{frame}{Segmentation}{Algorithm Overview}
  \begin{itemize}
    ...
    \vfill
    \item In each round, all pieces not yet assigned to a segment are assembled as if they come from a single input image.
    \vfill
    \item Segments of sufficient size are saved to be used in future Mixed-Bag Solver stages
    \vfill
    \item A piece can be assigned to at most a single segment
    \vfill
  \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%
