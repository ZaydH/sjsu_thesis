...
Whenever any piece, including the seed, is added to a segment, the algorithm examines all of that piece's neighbors.  These adjacent pieces are also added to the segment if they are in the pool of unassigned pieces and if their neighbor inside the segment is a ``best buddy'' (as determined by the predicate ``\textproc{IsBestBuddies}'').  The growth of a segment terminates when there are no neighboring pieces that satisfy these two criteria. 
...
\Function{HierarchicalClustering}{$solved\_segments, overlap\_matrix$}
	...
	\ForEach{$\text{segment } \Phi_i \in \textit{solved\_segments}$}
