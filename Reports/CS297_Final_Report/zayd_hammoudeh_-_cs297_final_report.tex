...
Figure~\ref{fig:directAccuracyOnePieceEffect} shows an original image and an actual solver output.  Note that only a single piece is misplaced, causing all pieces to be shifted over one location to the right.  This single misplaced piece causes the direct accuracy score to drop to 0\%.  Had this same piece been misplaced at either the right or bottom side of the image, the direct accuracy would have been largely unaffected.  The fact that direct accuracy can give such vastly differing results for essentially the same error indicates that it has room for improvement.
...
(a) Original Image
\\ ~\\
\fbox{\includegraphics[width=54.17mm]{./images/muffins_300x300_type1}}
\\
(b) Solver Output
