...
Paikin \& Tal \cite{paikin2015} published in 2015 a greedy solver that handles both Type~1 and Type~2 puzzles, even if those puzzles have missing pieces.  What is more, their algorithm is one of the first to support solving multiple jig swap puzzles simultaneously.  This entails mixing the pieces from multiple different puzzles together into a single, merged set; hence, the algorithm has no information about the source puzzle for each piece.  The solver then attempts to reconstruct the original set of distinct puzzles from the bag of pieces.
...
To predict the ideal value of the pixel on the other side of the puzzle piece boundary, this equation subtracts from the each border pixel the difference between that pixel and its neighbor in the adjacent column.  This allows Pomeranz \textit{et al.}'s predictive compatibility to account for local gradients across puzzle pieces.
...
Paikin \& Tal set $p$ and $q$ from Equation~\eref{eq:pomeranzPairwise} equal to 1 as this both increased the accuracy of their solver and significantly reduced time required to compute interpiece distance.  In a similar fashion to Equation~{eq:pomeranzPredCompat}, Paikin \& Tal's distance measure uses the pixels in column $K-1$ of puzzle piece $x_i$ to better predict the value of the pixels in piece $x_j$.  Since Equation~\eref{eq:paikinAsymDistance} does not consider the pixels in column $2$ of piece $x_j$, the distance is asymmetric; therefore, in most cases, $D(x_i,R,x_j,L)$ does not equal $D(x_j,L,x_i,R)$.

Pomeranz \textit{et. al.} only considered the relationship between two individual pieces when determining pairwise compatibility.  In images (or areas of images) with little variation (e.g., an all white image), non-adjacent puzzle pieces may have artificially low pairwise distances.  Paikin \& Tal's asymmetric compatibility, shown in Equation~\eref{eq:paikinAsymCompatibility}, corrects for this.  $secondD(x_i,L)$ is the second best asymmetric distance between the right side of piece $x_i$ and all other pieces. 
